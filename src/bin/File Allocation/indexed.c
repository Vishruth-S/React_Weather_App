...
void indexed(struct Memory memory[], int mem_size, struct File files[], int n)
{
    srand(time(0));
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        if (files[i].len <= remaining_memory)
        {
            ...
            remaining_memory -= (files[i].len + 1);
            for (int j = 0; j < files[i].len; j++)
            {
                memory[files[i].start].index[j] = indices[j];
            }
        }
    }
}
...
int main()
{
    ...
    indexed(memory, mem_size, files, n);
    printf("\nAllocation status\n");
    printf("\nFilename\tAllocated\tStarting\tLength\tIndices\n");
    for (int i = 0; i < n; i++)
    {
        ...
        if (files[i].start != -1)
            ...
            printf("No\t\t");
        printf("%d\t\t%d\t", files[i].start, files[i].len);
        for (int j = 0; j < files[i].len; j++)
            printf("%d ", memory[files[i].start].index[j]);
        printf("\n");
    }
    printf("\n\nMemory\n");
